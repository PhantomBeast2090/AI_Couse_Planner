...
| `tests/api.test.js` | Real Express app on an ephemeral port: scoped `POST /run` (shape + `SELECTION_REQUIRED` with no selection + `400` on unknown algorithm), `POST /compare` (both sides + winner + structural scope equality), `POST /agent` (strategy + log + workload + scope), `GET /graph` (overview + scoped subgraph), `GET /health`. |
| `tests/scope.test.js` | `resolvePlanningScope` contract: selection validation, program curriculum + closure, target-course ancestor closure, unrelated exclusion, scoped edges, capacity/depth stats, determinism, never a global fallback. |
